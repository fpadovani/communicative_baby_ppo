...
create_repo("fpadovani/rfsem1-baby", exist_ok=True)
...
for epoch in range(10):
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Epoch {epoch + 1}")):
        ...
        rewards = reward_fn_sem_similarity(prompts_batch, decoded_responses, teacher_refs_batch)
        ...
        if global_step % SAVE_EVERY == 0:
            output_dir = f"./fine_tuned_models/rfsem1-baby/checkpoint-{global_step}"
            ppo.model.save_pretrained(output_dir)
            tokenizer_baby.save_pretrained(output_dir)

            upload_folder(
            repo_id="fpadovani/rfsem1-baby",
            folder_path=f"./fine_tuned_models/rfsem1-baby/checkpoint-{global_step}",
...
# Save locally
output_dir = "./fine_tuned_models/rfsem1-baby"
ppo.model.save_pretrained(output_dir)
tokenizer_baby.save_pretrained(output_dir)

create_repo("fpadovani/rfsem1-baby", exist_ok=True)
ppo.model.push_to_hub("fpadovani/rfsem1-baby")
tokenizer_baby.push_to_hub("fpadovani/rfsem1-baby")